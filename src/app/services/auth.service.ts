...
export class AuthService {
  ...
  async loginByAuth({ email, password }) {
    try {
      ...
      this.http.post(this.config.apiBaseUrl+ Constants.AUTH_LOGIN_PEMOHON, bodyreq).subscribe((result: any) => {
        let d = result.data;
  ...
  async registerByAuth({ emailAddress, password, namaLengkapTanpaGelar, nomorAnggotaAsosiasi, role }) {
    ...
    try {
      ...
      this.http.post(this.config.apiBaseUrl+ Constants.AUTH_REGISTER_PEMOHON, bodyreq).subscribe(data => {
        console.log(data);
  ...
  async loginByHris({ email, password }) {
    try {
      ...
      console.log({ email, password });
      this.http.post(this.config.apiBaseUrl+ "api/Users/loginHRIS", bodyreq).subscribe((result: any) => {
        let d = result.data;
        localStorage.setItem('token', JSON.stringify(d).replace('"', '').replace('"', ''));
        console.log("token 1: ", localStorage.getItem('token'));
        let jwt = jwt_decode(localStorage.getItem('token'));
