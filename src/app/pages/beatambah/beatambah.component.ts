...
export class BeatambahComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.isEditMode || this.isPreview) {
      ...
      this.http.get(this.config.apiBaseUrl+ "api/LaporanPenyetoranBeaLelang/" + id, this.api.generateHeader()).subscribe((result: any) => {
  ...
  loadTrans() {
    this.http.get(this.config.apiBaseUrl+ "api/TransaksiLelang/" + this.idtrans, this.api.generateHeader()).subscribe((result: any) => {
  ...
  savetransaksi() {
    if (confirm("Apakah anda sudah mengisi data dengan lengkap dan benar?")) {
      this.http.post(this.config.apiBaseUrl+ "api/LaporanPenyetoranBeaLelang", this.generateBodyReq(this.beaForm.value), this.api.generateHeader()).subscribe(data => {
  ...
  submitFile() {
    this.uploadService.submitFile(this.config.apiBaseUrl+ "api/LaporanPenyetoranBeaLelang/uploadDokumen", this.fileUpload).subscribe((r) => {
      this.prog = r[1]
