...
export class TransaksidetailComponent implements OnInit,OnDestroy {
  ...
  loadTransaction()  {
    ...
    this.http.get(this.config.apiBaseUrl+"api/TransaksiLelang"+url, this.api.generateHeader()).subscribe((result: any) => {
      if (result.data) {
        this.listTrans = this.isP2pk?result.data:result.data.filter(trans => [this.idjadwal].includes(trans.jadwalLelangId))
