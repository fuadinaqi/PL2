...
export class BphdetailComponent implements OnInit {
  ...
  onLoadData(){
    ...
    this.http.get(this.config.apiBaseUrl+"api/LaporanRisalahLelangPengenaanBPHTB"+url, this.api.generateHeader()).subscribe((result: any) => {

      //this.listTrans = result.data
      this.listTrans = this.isP2pk?result.data:result.data.filter(trans => [this.idtrans].includes(trans.transaksiLelangId))
      console.log(result)
  ...
  onKirim(idtrans) {
    if (confirm("Apakah anda yakin ingin mengirim data?")) {
      console.log(idtrans);
      let url = this.isP2pk?"api/LaporanRisalahLelangPengenaanBPHTB/P2PK/BukaAkses":"api/LaporanRisalahLelangPengenaanBPHTB/Kirim"
      let msg = this.isP2pk?"BPHTB Terkirim ke Pejabat Lelang II":"BPHTB Terkirim ke Back Office PPPK"
      this.http.put(this.config.apiBaseUrl+url+"?id=" + idtrans, null, this.api.generateHeader()).subscribe(data => {
